...
local date = require("snaker.lib.date")
...
local function _persist_rule(info, rules)
  local success, err, forcible = snaker_db.set("consul_balancer.selector." .. info.id .. ".rules", rules)
...
local function _check(ctx, service_index)
  local args = {
    index = service_index,
    wait = snaker.data.config.consul.interval
  }
