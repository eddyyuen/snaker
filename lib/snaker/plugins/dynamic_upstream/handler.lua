...
local judge_util = require("snaker.utils.judge")
local extractor_util = require("snaker.utils.extractor")
local handle_util = require("snaker.utils.handle")
...
local function ngx_set_uri(uri,rule_handle)
    ...
    ngx.log(ngx.INFO, '[DynamicUpstream][upstream request][http://',ngx.var.upstream_scheme , ngx.var.upstream_url, ngx.var.upstream_request_uri, ']')
end

local function filter_rules(sid, plugin, ngx_var_uri)
    -- local rules = snaker_db.get_json(plugin .. ".selector." .. sid .. ".rules")
    local rules = snaker_db.get(plugin .. ".selector." .. sid .. ".rules")


    if not rules or type(rules) ~= "table" or #rules <= 0 then
        ngx.log(ngx.INFO, "==[DynamicUpstream] stop rules")
        return false
    end
...
function DynamicUpstreamHandler:rewrite(conf)
    ...
    local enable = snaker_db.get("dynamic_upstream.enable")
    -- local meta = snaker_db.get_json("dynamic_upstream.meta")
    -- local selectors = snaker_db.get_json("dynamic_upstream.selectors")

    local meta = snaker_db.get("dynamic_upstream.meta")    
    local selectors = snaker_db.get("dynamic_upstream.selectors")
