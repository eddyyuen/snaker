...
return function(plugin)
        ...
        POST = function(store)
            return function(req, res, next)
                ...
                if update_result then
                    local success, _, _ = snaker_db.set(plugin .. ".enable", enable)
                    if success then
                        return res:json({
                            success = true ,
    ...
    -- get config in snaker's node now
    API["/" .. plugin .. "/config"] = {
        GET = function(store)
            return function(req, res, next)
                local enable = snaker_db.get(plugin .. ".enable") or false
                local meta = snaker_db.get(plugin .. ".meta") or {}

                local selectors = {}
                if meta and meta.selectors and type(meta.selectors)=="table" then
                    for i, sid in ipairs(meta.selectors) do
                        local selector = {}
                        local cache_selectors = snaker_db.get(plugin .. ".selectors") or {}
                        for j, selector_detail in pairs(cache_selectors) do
                            if j == sid then
                                ...
                                if selector_detail.rules and type(selector_detail.rules) == "table" then
                                    ...
                                    local cache_rules = snaker_db.get(plugin .. ".selector." .. sid .. ".rules") or {}
                                    local rules = {}
    ...
    API["/" .. plugin .. "/selectors/:id/rules"] = {
        ...
        GET = function(store)
            return function(req, res, next)
                ...
                local rules = snaker_db.get(plugin .. ".selector." .. selector_id .. ".rules") or {}
                res:json({
                    success = true,
                    data = {
                        rules = rules
        ...
        PUT = function(store) -- modify
            return function(req, res, next)
                ...
                if update_result then
                    local old_rules = snaker_db.get(plugin .. ".selector." .. selector_id .. ".rules") or {}
                    local new_rules = {}
                    ...
                    local success, err, forcible = snaker_db.set(plugin .. ".selector." .. selector_id .. ".rules", new_rules)
    ...
    API["/" .. plugin .. "/selectors"] = {
        GET = function(store) -- get selectors
            return function(req, res, next)
                local meta = snaker_db.get(plugin .. ".meta")
                res:json({
                    success = true,
                    data = {
                        enable = snaker_db.get(plugin .. ".enable"),
                        meta = snaker_db.get(plugin .. ".meta"),
                        selectors = snaker_db.get(plugin .. ".selectors")
                    }
                })
            end
        end,
        ...
        DELETE = function(store) -- delete selector
            ...
            return function(req, res, next)
               ...
               --[[  if plugin == "consul_balancer" then
                    snaker.data.consul.remove_watch(to_del_selector)
                end ]]
